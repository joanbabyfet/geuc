...
class mod_news_cat extends mod_model
{
    //主键
    ...
    protected function list_data(array $conds)
    {
        ...
        $rows = self::get_list([
            'fields'    => ['id', 'name', 'desc', 'sort', 'status', 'create_user', 'create_time'
            ],
            'where'     => $where,
            'page'      => $page,
            'page_size' => $page_size,
    ...
    protected function make_menu(array $data)
    {
        ...
        //獲取商品分類樹
        $news_cat_tree = self::list_data([
            'status'    => mod_news_cat::ENABLE,
            'order_by'  => ['create_time', 'asc']
        ]);
        return outToHtml($news_cat_tree, route('web.news.index')."?cat_id=");
    }
}
