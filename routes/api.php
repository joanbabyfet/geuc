...
Route::post('login', 'ctl_member@login')->name('api.member.login');
Route::post('register', 'ctl_member@register')->name('api.member.register');
Route::get('ping', 'ctl_common@ping')->name('api.common.ping');
Route::get('ip', 'ctl_common@ip')->name('api.common.ip');
Route::post('refresh_token', 'ctl_member@refresh_token')->name('api.member.refresh_token'); //刷新认证token
Route::get('h5', 'ctl_h5@detail')->name('api.h5.detail');

//目前改用jwt.auth,通过auth中间件并指定api守卫
Route::group(['middleware' => ['jwt.auth']], function() {
    Route::post('logout', 'ctl_member@logout')->name('api.member.logout'); //退出
    Route::post('member_info', 'ctl_member@detail')->name('api.member.detail'); //用户信息
