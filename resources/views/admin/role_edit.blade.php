<div class="layui-card-body ">
    <form id="layer-form" class="layui-form" action="{{ route("admin.role.edit") }}" method="POST">
        {{ csrf_field() }}
        <input type='hidden' name='id' value="{{ $row['id'] }}"/>
        <div class="layui-form-item">
            <label class="layui-form-label label-required-next">用戶組名:</label>
            <div class="layui-input-block">
                <input type="text" name="name" value="{{ $row['name'] }}" placeholder="請輸入用戶組名"
                                            ...
                                            @foreach($group['children'] as $permission)
                                                <div class="layui-col-sm4">
                                                    <input type="checkbox"
                                                        ...
                                                        {{ ($purviews == ['*'] || in_array($permission['id'], $purviews)) ? ' checked':'' }}>
                                                </div>
                                            @endforeach
                                        @endif
                                    </div>
