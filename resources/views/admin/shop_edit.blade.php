<div class="layui-card-body ">
    <form id="layer-form" class="layui-form" action="{{ route("admin.shop.edit") }}" method="POST">
        {{ csrf_field() }}
        ...
        <div class="layui-form-item">
            <label class="layui-form-label label-required-next">商家/公司名稱:</label>
            <div class="layui-input-block">
                <input type="text" name="name" value="{{ $row['name'] }}" placeholder="請輸入商家/公司名稱"
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline" style="width: 100px;">
                <select id="country_id" name="country_id" lay-filter="country_id"
                        lay-verify="required">
                    <option value=""></option>
                    {!! make_options($country_options, $row['country_id']) !!}
                </select>
            </div>
            <div class="layui-input-inline" style="width: 100px;">
                <select id="province_id" name="province_id" lay-filter="province_id"
                        lay-verify="required">
                    <option value=""></option>
                    {!! make_options($province_options, $row['province_id']) !!}
                </select>
            </div>
            <div class="layui-input-inline" style="width: 100px;">
                <select id="city_id" name="city_id" lay-filter="city_id"
                        lay-verify="required">
                    <option value=""></option>
                    {!! make_options($city_options, $row['city_id']) !!}
                </select>
            </div>
            <div class="layui-input-inline" style="width: 100px;">
                <select id="area_id" name="area_id" lay-filter="area_id"
                        lay-verify="">
                    <option value=""></option>
                    {!! make_options($area_options, $row['area_id']) !!}
                </select>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label label-required-next">商家地址:</label>
            <div class="layui-input-block">
                <input type="text" name="address" value="{{ $row['address'] }}" placeholder="請輸入商家地址"
                       class="layui-input" required lay-verify="required">
            </div>
        </div>
        <div class="layui-form-item uploader-group uploader-group-img"
            ...
            <label class="layui-form-label">經營證件照:</label>
            <div class="layui-input-block">
                <!--用来存放文件信息-->
                <div class="uploader-list">
                    @foreach($row['license_dis'] as $k => $v)
                        <div id="WU_FILE_{{ $k + 10 }}" class="item img-item pull-left" style="margin-bottom:10px;margin-right:10px;">
                            <img style="width:100px;height:100px;" src="{{ $row['license_url_dis'][$k] }}">
                            <i class="fa fa-close close-btn"></i>
                            <div class="layui-progress" lay-filter="pro" lay-showPercent="true" style="display: none;">
                                <div class="layui-progress-bar" lay-percent="100%"></div>
                            </div>
                            <input type="hidden" name="license[]" value="{{ $v }}" class="hid-filename">
                        </div>
                    @endforeach
                </div>
                <a class="layui-btn layui-btn-sm layui-btn-primary uploader-picker" data-file="license[]" data-type="image">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="contact" value="{{ $row['contact'] }}" placeholder="請輸入聯絡人"
                       class="layui-input" required lay-verify="required">
            </div>
            <div class="layui-input-inline">
                <input type="radio" name="sex" value="1" title="男" {{ ($row['sex'] == 1) ? ' checked':'' }} />
                <input type="radio" name="sex" value="0" title="女" {{ ($row['sex'] == 0) ? ' checked':'' }} />
            </div>
        </div>
        <div class="layui-form-item">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="status" lay-verify="required">
                    <option value=""></option>
                    {!! make_options($status_options, $row['status']) !!}
                </select>
            </div>
        </div>
        <div class="layui-form-item">
