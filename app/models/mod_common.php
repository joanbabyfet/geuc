...
class mod_common extends Model
{
     ...
     * @return array
     */
    public static function make_level(array $data, $pid = 0, $level = 0, $field_id = 'id', $field_pid = 'pid', $clear = true)
    {
        ...
        foreach($data as $v)
        {
            if(isset($v[$field_pid]) && $v[$field_pid] == $pid)
            {
                $v['level'] = $level;
                $arr[] = $v;
                self::make_level($data, $v[$field_id], $level+1,
     ...
     * @return array
     */
    public static function get_all_child(array $data, $pid = 0, $level = 0, $field_id = 'id', $field_pid = 'pid')
    {
        $arr = [];
        //遍历
        foreach($data as $v)
        {
            if(isset($v[$field_pid]) && $v[$field_pid] == $pid)
            {
                $v['level'] = $level;
                $arr[] = $v;
                $arr = array_merge($arr, self::get_all_child($data, $v[$field_id], $level + 1,
                    $field_id, $field_pid));
            }
        }
        return $arr;
     ...
     * @return array
     */
    public static function get_all_parent(array $data, $id = 0, $level = 0, $field_id = 'id', $field_pid = 'pid', $clear = true)
    {
        //static的作用仅在第一次调用函数时对变量进行初始化,并保留变量值
        static $arr = [];
        //首次进入清除上次调用函数留下的静态变量的值，进入深一层循环时则不要清除。
        ...
        foreach($data as $v)
        {
            if(isset($v[$field_id]) && $v[$field_id] == $id)
            {
                $v['level'] = $level;
                $arr[] = $v;
                self::get_all_parent($data, $v[$field_pid], $level-1,
                    $field_id, $field_pid, false);
            }
        }
        return $arr;
     ...
     * @return array
     */
    public static function get_all_parent_ids(array $data, $id)
    {
        $rows = self::get_all_parent($data, $id);
     ...
     * @return array
     */
    public static function get_all_child_ids(array $data, $pid)
    {
        $rows = self::get_all_child($data, $pid);
        $ids = array_column($rows, 'id');
        sort($ids);
        return $ids;
    }
     ...
     * 获取多语言字段名
     * @param $field 字段名
     * @param string $lang 指定语言 zh-tw/en/km
     * @return string
     */
    public static function get_lang_field($field, $lang='')
    {
        $lang = empty($lang) ? (empty($GLOBALS['lang']) ? 'zh-tw' : $GLOBALS['lang']) : $lang;
        $ret =  ($lang === 'zh-tw') ? $field : "{$field}_{$lang}";
        return $ret;
    }
