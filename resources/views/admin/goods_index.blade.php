    ...
    <div class="layui-card-body ">
        <form class="layui-form layui-col-space5" id="search-form" method="GET">
            <div class="layui-inline layui-show-xs-block">
                <input type="text" name="name" placeholder="請輸入商品名稱" value="{{ request("name") }}" autocomplete="off"
                       class="layui-input">
            </div>
            <div class="layui-inline layui-show-xs-block">
    ...
    <div class="layui-card-body ">
        <script type="text/html" id="toolbar">
            <div class="layui-btn-container">
                <a class="layui-btn layui-btn-sm" onclick="admin.openLayerForm('{{ route("admin.goods.add") }}', '商品添加', 'POST', '700px', '400px')"><i class="layui-icon"></i>新增</a>
        ...
        <table id="list" lay-filter="list" style="display: none"
            ...
            <tr>
                ...
                <th lay-data="{field:'name'}">商品名稱</th>
                <th lay-data="{field:'store_name_dis'}">所屬店舖</th>
                <th lay-data="{field:'goods_cat_name_dis'}">商品分類</th>
            ...
            @foreach ($list as $k => $v)
                <tr>
                    <td>{{ $v['id'] }}</td>
                    <td>{{ $v['name'] }}</td>
                    <td>{{ $v['store_name_dis'] }}</td>
                    <td>{{ $v['goods_cat_name_dis'] }}</td>
                    <td>{{ $v['status_dis'] }}</td>
                    <td>{{ $v['create_time_dis'] }}</td>
                    <td>
                        <a class="layui-btn layui-btn-xs"
                           onclick="admin.openLayerForm('{{ route("admin.goods.edit").'?id='.$v['id'] }}', '商品編輯', 'POST', '700px', '400px')">編輯</a>
                        <a class="layui-btn layui-btn-xs layui-btn-danger"
                           onclick="admin.tableDataDelete('{{ route("admin.goods.delete").'?ids[]='.$v['id'] }}', this)">刪除</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
            ...
            table.on('toolbar(list)', function(obj){
                ...
                switch(obj.event)
                {
                    ...
                    case 'export':
                        ...
                        _data.fields.push('store_name_dis');
                        _data.fields.push('goods_cat_name_dis');
                        _data.fields.push('status_dis');
                        _data.fields.push('create_time_dis');
                        export_excel('{{ route('admin.goods.export_list') }}', _data, search_str);
                        break;
                }
            });
