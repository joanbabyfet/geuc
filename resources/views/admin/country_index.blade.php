    ...
    <div class="layui-card-body ">
        <script type="text/html" id="toolbar">
            <div class="layui-btn-container">
                <a class="layui-btn layui-btn-sm" onclick="admin.openLayerForm('{{ route("admin.country.add") }}', '國家添加', 'POST', '500px', '350px')"><i class="layui-icon"></i>新增</a>
        ...
        <table id="list" lay-filter="list" style="display: none"
            ...
            <tr>
                ...
                <th lay-data="{field:'name'}">國家名稱</th>
                <th lay-data="{field:'en_short_name'}">英文簡稱</th>
                <th lay-data="{field:'is_default_dis'}">默認展示</th>
                <th lay-data="{field:'is_fix_dis'}">固定頻道</th>
            ...
            @foreach ($list as $k => $v)
                <tr>
                    <td>{{ $v['id'] }}</td>
                    <td>{{ $v['name'] }}</td>
                    <td>{{ $v['en_short_name'] ?: '-' }}</td>
                    <td>{{ $v['is_default_dis'] }}</td>
                    <td>{{ $v['is_fix_dis'] }}</td>
                    <td>{{ $v['status_dis'] }}</td>
                    <td>
                        <a class="layui-btn layui-btn-xs"
                           onclick="admin.openLayerForm('{{ route("admin.country.edit").'?id='.$v['id'] }}', '國家編輯', 'POST', '500px', '350px')">編輯</a>
                        <a class="layui-btn layui-btn-xs layui-btn-danger"
                           onclick="admin.tableDataDelete('{{ route("admin.country.delete").'?ids[]='.$v['id'] }}', this)">刪除</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
            ...
            table.on('toolbar(list)', function(obj){
                ...
                switch(obj.event)
                {
                    ...
                    case 'batch_enable': //批量啟用
                        if(flag !== 0){
                            layui.layer.confirm('你確認啟用嗎？', {icon: 3, title: "提示", btn: ['確定', '取消']}, function (){
                                $.ajax({
                                    url: '{{ route('admin.country.enable') }}',
                                    type: 'POST',
                                    data: { ids: ids }, //送到服务器數據
                                    success: function(res) {
                                        if (res.code === 0) {
                    ...
                    case 'batch_disable': //批量禁用
                        if(flag !== 0){
                            layui.layer.confirm('你確認禁用嗎？', {icon: 3, title: "提示", btn: ['確定', '取消']}, function (){
                                $.ajax({
                                    url: '{{ route('admin.country.disable') }}',
                                    type: 'POST',
                                    data: { ids: ids }, //送到服务器數據
                                    success: function(res) {
                                        if (res.code === 0) {
                    ...
                    case 'export':
                        ...
                        _data.fields.push('en_short_name');
                        _data.fields.push('is_default_dis');
                        _data.fields.push('is_fix_dis');
                        export_excel('{{ route('admin.country.export_list') }}', _data, search_str);
                        break;
                }
            });
