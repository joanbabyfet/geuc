    ...
    <div class="layui-card-body ">
        <form class="layui-form layui-col-space5" id="search-form" method="GET">
            <div class="layui-inline layui-show-xs-block">
                <input type="text" name="name" placeholder="請輸入短信名稱" value="{{ request("name") }}" autocomplete="off"
                       class="layui-input">
            </div>
            <div class="layui-inline layui-show-xs-block">
    ...
    <div class="layui-card-body ">
        <script type="text/html" id="toolbar">
            <div class="layui-btn-container">
                <a class="layui-btn layui-btn-sm" onclick="admin.openLayerForm('{{ route("admin.sys_sms.add") }}', '發送短信', 'POST', '650px', '500px')"><i class="layui-icon"></i>發送短信</a>
        ...
        <table id="list" lay-filter="list" style="display: none"
            ...
            <tr>
                ...
                <th lay-data="{field:'object_type_dis'}">發送對象</th>
                <th lay-data="{field:'name'}">短信名稱</th>
                <th lay-data="{field:'content'}">短信內容</th>
                <th lay-data="{field:'send_time_dis'}">發送時間</th>
            </tr>
            </thead>
            <tbody>
            @foreach ($list as $k => $v)
                <tr>
                    <td>{{ $v['id'] }}</td>
                    <td>{{ $v['object_type_dis'] }}</td>
                    <td>{{ $v['name'] ?: '-' }}</td>
                    <td>{{ $v['content']?: '-' }}</td>
                    <td>{{ $v['send_time_dis'] ?: '-' }}</td>
                </tr>
            @endforeach
            </tbody>
        </table>
            ...
            table.on('toolbar(list)', function(obj){
                ...
                switch(obj.event)
                {
                    case 'batch_del': //批量刪除
                        if(flag !== 0){
                            layui.layer.confirm('你確認刪除嗎？', {icon: 3, title: "提示", btn: ['確定', '取消']}, function (){
                                $.ajax({
                                    url: '{{ route('admin.sys_sms.delete') }}',
                                    type: 'POST',
                    ...
                    case 'export':
                        ...
                        var _data = {fields:[]};
                        _data.fields.push('object_type_dis');
                        _data.fields.push('name');
                        _data.fields.push('content');
                        _data.fields.push('send_time_dis');
                        export_excel('{{ route('admin.sys_sms.export_list') }}', _data, search_str);
                        break;
                }
            });
