     ...
     $.ajax({
        ...
        success: function(res) {
            if (res.code === 0)
            {
                ...
                var t = ((_page_no/_total_page)*100).toFixed();
                layui.element.progress('pro', t + '%');
                if(_page_no >= _total_page){
                    setTimeout(function(){ //N秒後只調用一次函數,setTimeout()只執行一次，setInterval()可以執行多次
                        layui.layer.close(index);
