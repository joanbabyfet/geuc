...
if(!function_exists('outToHtml'))
{
    /**
     * 生成html菜單
     * @param array $tree
     * @param string $url
     * @param string $type 樣式
     * @return string
     */
    function outToHtml(array $tree, $url = '', $type = '1')
    {
        $html = '';
        foreach($tree as $k => $t)
        {
            if($type == '1')
            {
                $uri = empty($url) ? 'javascript:;' : $url.$t['id'];
                if(!isset($t['children'])) //没有子分类
                {
                    $html .= "<li><a href='{$uri}'>".$t['name']."</a></li>\n";
                }
                else
                {
                    $html .="<li><a href='{$uri}'>".$t['name']."</a>\n<ul>\n";
                    $html .= outToHtml($t['children'], $url);
                    $html  = $html."</ul>\n</li>";
                }
            }
        }
